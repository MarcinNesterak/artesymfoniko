  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const participation = eventDetails.participations.find(
          (p) => p._id === participationId
        );
        if (!participation) throw new Error("Nie znaleziono uczestnictwa");

        const musicianDetails = await usersAPI.getMusician(
          participation.userId._id
        );
        const eventDate = new Date(eventDetails.event.date)
          .toISOString()
          .split("T")[0];
        ...
        setContractData((prev) => ({
          ...
          numerUmowy: `UOD/${eventDetails.event.title.replace(
            /\s+/g,
            "-"
          )}/${participation.userId.name.replace(
            /\s+/g,
            "_"
          )}/${new Date().getFullYear()}`,
          wykonawca: {
            imieNazwisko: musicianDetails.user.name,
            adres: musicianDetails.user.personalData?.address
              ? `${musicianDetails.user.personalData.address.street}, ${musicianDetails.user.personalData.address.postalCode} ${musicianDetails.user.personalData.address.city}`
              : "",
            pesel: musicianDetails.user.personalData?.pesel || "",
            numerKonta:
              musicianDetails.user.personalData?.bankAccountNumber || "",
          },
    ...
    <div className="contract-generator-container">
      <div className="form-section">
        ...
        <div className="form-group">
          ...
          <input
            ...
            id="nazwa"
            name="nazwa"
            value={contractData.zamawiajacy.nazwa}
            onChange={(e) => handleNestedChange("zamawiajacy", e)}
          />
        </div>
        <div className="form-group">
          ...
          <input
            ...
            id="adres"
            name="adres"
            value={contractData.zamawiajacy.adres}
            onChange={(e) => handleNestedChange("zamawiajacy", e)}
          />
        </div>
        <div className="form-group">
          ...
          <input
            ...
            id="nip"
            name="nip"
            value={contractData.zamawiajacy.nip}
            onChange={(e) => handleNestedChange("zamawiajacy", e)}
          />
        </div>
        <div className="form-group">
          ...
          <input
            ...
            id="regon"
            name="regon"
            value={contractData.zamawiajacy.regon}
            onChange={(e) => handleNestedChange("zamawiajacy", e)}
          />
        </div>
        <div className="form-group">
          ...
          <input
            ...
            id="reprezentant"
            name="reprezentant"
            value={contractData.zamawiajacy.reprezentant}
            onChange={(e) => handleNestedChange("zamawiajacy", e)}
          />
        </div>
        <div className="form-group">
          <label htmlFor="wynagrodzenieBrutto">
          ...
          <input
            ...
            value={contractData.wynagrodzenieBrutto}
      ...
      <div className="preview-section">
        <div className="page-break">
          ...
          <p>
            <strong>
              {contractData.miejsceZawarcia}, {contractData.dataWykonaniaDziela}
            </strong>
          </p>
          <hr />
          ...
          <h3>Zamawiający:</h3>
          <p>
            <strong>{contractData.zamawiajacy.nazwa}</strong>
            <br />z siedzibą przy {contractData.zamawiajacy.adres},<br />
            NIP: {contractData.zamawiajacy.nip}, REGON:{" "}
            {contractData.zamawiajacy.regon},<br />
            zwanym dalej Zamawiającym.
          </p>

          <h3>Wykonawca:</h3>
          <p>
            <strong>{contractData.wykonawca.imieNazwisko}</strong>
            <br />
            zamieszkały/a pod adresem {contractData.wykonawca.adres},<br />
            posługujący/a się numerem PESEL: {contractData.wykonawca.pesel},
            <br />
            zwanym/ą dalej Wykonawcą
          </p>

          <p>
            Dzieło zostało wykonane w dniu {contractData.dataWykonaniaDziela}.
          </p>
          ...
          <table>
            <tbody>
              <tr>
                ...
                <td>{contractData.wynagrodzenieBrutto} PLN</td>
              </tr>
              <tr>
                <td>Koszty uzyskania przychodu (50%)</td>
                <td>{contractData.kosztyUzyskaniaPrzychodu} PLN</td>
              </tr>
              <tr>
                <td>Podstawa naliczenia podatku dochodowego</td>
                <td>{contractData.podstawaOpodatkowania} PLN</td>
              </tr>
              <tr>
                <td>Należna zaliczka na podatek dochodowy</td>
                <td>{contractData.zaliczkaNaPodatek} PLN</td>
              </tr>
              <tr>
                <td>
                ...
                <td>
                  <strong>{contractData.wynagrodzenieNetto} PLN</strong>
                </td>
              </tr>
              <tr>
                <td>
          ...
          <p>
            ...
            {contractData.wykonawca.numerKonta}
          </p>
        ...
        <div className="page-break">
          ...
          <p>
            <strong>
              {contractData.miejsceZawarcia}, {contractData.dataZawarcia}
            </strong>
          </p>
          <p>Zawarta w dniu {contractData.dataZawarcia} pomiędzy:</p>
          <p>
            <strong>{contractData.zamawiajacy.nazwa}</strong>, z siedzibą przy{" "}
            {contractData.zamawiajacy.adres}, NIP:{" "}
            {contractData.zamawiajacy.nip}, REGON:{" "}
            {contractData.zamawiajacy.regon}, reprezentowana przez{" "}
            {contractData.zamawiajacy.reprezentant} zwanego dalej Zamawiającym.
          </p>
          <p>a</p>
          <p>
            <strong>{contractData.wykonawca.imieNazwisko}</strong>,
            zamieszkały/a pod adresem {contractData.wykonawca.adres},
            posługujący/a się numerem PESEL: {contractData.wykonawca.pesel},
          ...
          <p>
            ...
            polegające na: {contractData.przedmiotUmowy}
          </p>

          <h2>§2 - Czas trwania umowy</h2>
          <p>
            Termin rozpoczęcia prac strony ustaliły na{" "}
            {contractData.dataZawarcia}, a termin ukończenia dzieła na{" "}
            {contractData.dataWykonaniaDziela}.
          </p>
          ...
          <ol>
            <li>
              ...
              <strong>{contractData.wynagrodzenieBrutto}</strong> (słownie:{" "}
              {amountToWords(contractData.wynagrodzenieBrutto)}).
            </li>
            <li>
          ...
          <ol>
            <li>
              ...
              {contractData.zamawiajacy.nazwa} z siedzibą w{" "}
              {contractData.zamawiajacy.adres.split(",")[0]}...
            </li>
          </ol>
          ...
          <div className="signatures">
            <div>
              ...
              <p>
                NIP {contractData.zamawiajacy.nip}, REGON{" "}
                {contractData.zamawiajacy.regon}
              </p>
              <p>Zamawiający</p>
            </div>
            <div>
